...
export default class DomainShow extends React.Component {
	constructor(props) {
		...
		for (let i = 0; i < this.props.item.concepts.length; i+=1) {
			items.push(<MenuItem 
				value={this.props.item.concepts[i]} key={i} 
				primaryText={this.props.item.concepts[i]} />);
		}
		return(
			<div>
			<Snackbar
			...
			iconButtonElement={<IconButton 
				style={{paddingBottom:0}}><MoreHorzIcon style={{color:'grey'}}/>
				</IconButton>}
				...
				leftIcon={<ActionInfo/>} onTouchTap = {this.handleTouchTap}/>
				<MenuItem primaryText='Add Documents' 
				leftIcon={<ActionAddBox/>} onTouchTap = {this.handleOpenDocs}/>
				<MenuItem primaryText='ReIndex' 
				leftIcon={<NavigationRefresh/>}	onTouchTap={this.handleOpen}/>
				</IconMenu>
				</Col>
				</Row>
				</Card>
				...
				<h3 style={{paddingTop: 15,color: 'grey'}}>Select Concept</h3>
				</Col>
				<Col lg={6} md={6} sm={6} xs={6}>
					...
					<Row>
					<Col lg={12} md={12} sm={12} xs={12}>
					<h3 style={{paddingLeft: 15,color: 'grey'}}>Add Documents :</h3>
					</Col>
					</Row>
